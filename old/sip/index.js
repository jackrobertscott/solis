...
Sip.prototype.run = function run(tasks, opts, runCb) {
  ...
  if (typeof opts === 'function') {
    ...
    _this.options = opts || _this.options;
  } else if (opts) {
...
Sip.prototype._auto = function _auto(tasks, tree, _autoCb) {
  ...
  async.parallel(tasks.filter(function(task) {
    return !!!_this.options.ignore[task];
  }).map(function(name) {
    return function(taskCb) {
      ...
      // add function to task tree with deps for async.auto
      tree[name] = (_this.tasks[name].deps || []).filter(function(task) {
        return !!!_this.options.ignore[task];
      }).concat([function(treeCb) {
        ...
        var cb = function(err) {
          ...
          if (cbCalled) {
            err = new Error('task callback called more than 1 times');
            return treeCb(err);
          }
          cbCalled = true;
        ...
        if (_this.tasks[name].fn) {
          // run function
          var res = _this.tasks[name].fn(_this.options, cb);
          ...
          } else if (res && typeof res.pipe === 'function') {
            // wait for stream to end
            eos(res, {
              error: true,
              readable: res.readable,
              writable: res.writable && !res.readable
            }, cb);
