...
Sip.prototype.run = function run(tasks, opts, runCb) {
  ...
  if (typeof opts === 'function') {
    ...
    throw new TypeError(this._formatTypeError('options', 'object', 'sip.run'));
  }
  if (runCb && typeof runCb !== 'function') {
    throw new TypeError(this._formatTypeError('callback', 'function', 'sip.run'));
  }
  var tree = {};
...
Sip.prototype._auto = function _auto(tasks, tree, _autoCb) {
  ...
  async.parallel(tasks.map(function(name) {
    return function(taskCb) {
      ...
      tree[name] = (this.tasks[name].deps || []).concat([function(treeCb) {
