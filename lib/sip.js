...
Sip.prototype.watch = function watch(glob, opts, tasks, fn) {
  if (typeof glob !== 'string') {
    return typeError('glob', 'string', 'sip.watch');
  }
  if (typeof opts === 'string' || Array.isArray(opts)) {
    ...
    return typeError('options', 'object', 'sip.watch');
  }
  if (typeof tasks === 'function') {
    fn = tasks;
    tasks = null;
  } else if (tasks && typeof tasks !== 'string' && !Array.isArray(tasks)) {
    return typeError('tasks', 'string/array', 'sip.watch');
  }
  if (fn && typeof fn !== 'function') {
    return typeError('function', 'function', 'sip.watch');
  }
  if (!tasks && !fn) {
    return error('either tasks or function must be passed to sip.watch()');
  }
...
Sip.prototype.task = function task(name, deps, fn) {
  if (typeof name !== 'string') {
    return typeError('name', 'string', 'sip.task');
  }
  if (typeof deps === 'function') {
    ...
    return typeError('dependencies', 'array', 'sip.task');
  }
  if (fn && typeof fn !== 'function') {
    return typeError('function', 'function', 'sip.task');
  }
  if (!deps && !fn) {
    return error('either dependencies or function must be passed to sip.task()');
  }
...
Sip.prototype._auto = function _auto(tasks, tree, _autoCb) {
  if (typeof tasks === 'string') {
    ...
    return error('(string/array) tasks incorrectly passed to sip._auto()');
  }

  async.parallel(tasks.map(function(name) {
    return function(taskCb) {
      if (!this.tasks.hasOwnProperty(name)) {
        return error('task: ' + name + ' passed to sip._auto() does not exist');
      }
 ...
 * Type error.
 */
function typeError(param, type, fname) {
  console.log(chalk.red('Type Error:'), '(' + type + ')', param, 'incorrectly passed to', fname + '()');
}
