...
/**
 * @param {boolean} opts.serve - serve the dist folder after compile
 */
sip.task('dist', ['tmp', 'dist:clean'], function(opts, cb) {
  var runCb = (!opts.serve) ? cb : function() {
    sip.run('dist:browser-sync', cb);
  };
  sip.run('dist:compile', runCb);
});
 ...
 */
sip.task('dist:browser-sync', function(opts, cb) {
  browserSync.init({
    server: opts.dist,
  }, cb);
});
...
sip.task('dist:inject', function(opts) {
  ...
  return sip.src(path.join(opts.dist, '**/*.html'))
    .pipe(inject(sources, {
      relative: true,
    }))
    .pipe(inject(vendor, {
